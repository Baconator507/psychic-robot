 ...
 (make-holdemagent 
   ...
   :agentfunction #'(lambda (roundstate id)
                  (cond ((zerop (holdemround-bet roundstate))
                         (LIST :check)) ;; essentially, a call, but we agreed in class when required bet is zero, we must check.
...
;;
;;Illegal actions will be treated as FOLDS.
...
(defun preflop-stage (baseround seating deck &optional (verbose nil) (pauser nil))
  (let* ((stagedone nil)
...
(defun cleanup-stage (baseround seating deck &optional (verbose nil) (pauser nil))
  ...
  (loop for playerid in (holdemround-players baseround) doing
        (let ((gamestate (copy-holdemround baseround :reveal-unfolded t))
  ...
  (when verbose
    (format t "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$~%~%END OF ROUND: BANKROLLS ARE UPDATED, and NON-BUSTED PLAYERS NOTIFIED OF THIS STATE:")
